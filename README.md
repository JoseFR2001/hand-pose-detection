...
MediaPipe Hands funciona mediante un _pipeline_ optimizado de dos modelos cooperativos (enfoque de cascada):
...
### 7. Funcionalidad adicional: captura de postura prohibida

Este proyecto incorpora una función extra de seguridad / control: el usuario puede capturar una postura de mano en tiempo real y guardarla como "postura prohibida". Una vez registrada, el modelo compara continuamente la pose detectada con la postura prohibida y muestra una advertencia visual cuando coincide.

- La captura se realiza sobre una pose reconocida por el sistema, normalizando los 21 landmarks de la mano para que la comparación sea independiente de la escala y la posición.
- La comparación toma en cuenta tanto la postura original como su versión reflejada horizontalmente, lo que permite detectar la postura aunque la mano esté en espejo.
- El umbral de detección es configurable, lo que permite ajustar la sensibilidad de la advertencia de postura prohibida.
...
**Sí, absolutamente.** Estos modelos basados en redes neuronales convolucionales (CNN) ligeras y optimizadas para _Edge Computing_ (procesamiento en el dispositivo del cliente) son indispensables hoy en día para aplicaciones de baja latencia en navegadores web y smartphones.
